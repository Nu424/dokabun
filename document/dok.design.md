...
- **非構造化出力列**
  - `ns_{プロンプト}`: 列ごとにプレーンテキスト出力を生成し、その列のセルへ書き込む（列単位で LLM を呼び出し、まとめない）。
  - `nsf_{プロンプト}`: 生成結果をファイルへ保存し、セルには保存したファイル名（`output_dir` 相対）を書く。
    - デフォルトの命名: `nsf{nsf_index}_{データ行番号}.txt`（行番号はデータ行 1 始まり）
    - `t_` 列が 1 つで、その値が実在ファイルパスなら `{target_file_stem}_nsf{nsf_index}.txt` を使用。
    - 拡張子はオプション（`--nsf-ext`）で `txt` / `md` を選択可能。テンプレートは `--nsf-name-template` / `--nsf-name-template-filetarget` で上書き可能。
  - `ns_`/`nsf_` が存在する場合、構造化出力列が無くても処理対象になる。
- **分析対象セルとスキップ条件**
